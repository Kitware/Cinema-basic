...
def demonstrate_manual_populate(fname="/tmp/demonstrate_manual_populate/info.json"):
    ...
    cs.add_descriptor("theta", make_parameter('theta', thetas))
    cs.add_descriptor("phi", make_parameter('phi', phis))
...
def demonstrate_populate(fname="/tmp/demonstrate_populate/info.json"):
    ...
    cs = FileStore(fname)
    cs.filename_pattern = "{theta}/{phi}"
    cs.add_descriptor("theta", make_parameter('theta', [0,10,20,30,40]))
    cs.add_descriptor("phi", make_parameter('phi', [0,10,20,30,40]))
...
def test_vtk_clip(fname=None):
    ...
    cs.add_descriptor("offset", make_parameter('offset', [0,.2,.4,.6,.8,1.0]))
...
def test_pv_slice(fname):
    ...
    cs.add_descriptor("phi", make_parameter('phi', [90, 120, 140]))
    cs.add_descriptor("theta", make_parameter('theta', [-90,-30,30,90]))
    cs.add_descriptor("offset", make_parameter('offset', [-.4,-.2,0,.2,.4]))
    cs.add_descriptor("color", make_parameter('color', ['yellow', 'cyan', "purple"], typechoice='list'))
...
def test_pv_contour(fname):
    ...
    cs.add_descriptor("phi", make_parameter('phi', [90,120,140]))
    cs.add_descriptor("theta", make_parameter('theta', [-90,-30,30,90]))
    cs.add_descriptor("contour", make_parameter('contour', [50,100,150,200]))
    cs.add_descriptor("color", make_parameter('color', ['white', 'RTData'], typechoice='list'))
...
def test_NOP(fname):
    ...
    cs.add_descriptor("contour", make_parameter('contour', [50,100,150,200]))
    cs.add_descriptor("color", make_parameter('color', ['white', 'RTData'], typechoice='list'))
    cs.add_descriptor("operation", make_parameter('operation',
